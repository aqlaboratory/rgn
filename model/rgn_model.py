...
class RGNModel(object):
    ...
    def _create_graph(self, mode, config):
        ...
        with tf.variable_scope(SCOPE, reuse=(mode == 'evaluation')) as scope, \
            ...
            recurrent_outputs = _higher_recurrence(mode, recurrence_config, inputs, num_stepss, alphabet=alphabet)
            ...
            if config.loss['secondary_weight'] > 0:
                ...
                if mode == 'evaluation': 
                    prediction_ops.update({'ids': ids, 'dssps': dssps, 'num_stepss': num_stepss})
    ...
    def _predict(self, session):
        ...
        if RGNModel._is_started:
            ...
            predictions = {}
            for id_, num_steps, secondary, tertiary in izip_longest(*[prediction_dict.get(key, []) 
                                                                      for key in ['ids', 'num_stepss', 'dssps', 'coordinates']]):
                prediction = {}
    ...
    if config['higher_order_layers']:
        ...
        for layer_idx in range(num_layers):
            with tf.variable_scope('layer' + str(layer_idx)):
                ...
                layer_recurrent_outputs = _recurrence(mode, layer_config, layer_inputs, num_stepss)
    ...
    if 'Cudnn' in config['recurrent_unit']:
        ...
        for scope in scopes:
            with tf.variable_scope(scope):
                ...
                outputs_directed, _, = rnn(inputs_directed, training=is_training)
